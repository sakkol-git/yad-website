...
export function UserDonationsTable({ donations, totalDonated }: UserDonationsTableProps) {
  ...
  const headerActions = (
    <div className="bg-surface-container-low px-6 py-4 rounded-md border border-outline-variant/30 flex items-center justify-between gap-6 shadow-sm">
      <div className="flex flex-col">
        <span className="text-xs font-bold text-on-surface-variant uppercase tracking-wider mb-1">
          Total Contributed
        </span>
      ...
      <Button
        ...
        className="bg-tertiary text-on-tertiary hover:bg-tertiary/90 shadow-sm border-none rounded-md"
      >
        Donate Again
      </Button>
    </div>
  ...
  const columns: ColumnDef<UserDonation>[] = [
    ...
    },
    {
      ...
      header: "Amount",
      cell: (row) => (
        <span className="font-body-lg text-body-lg font-bold text-on-surface">${row.amount}</span>
      ),
    },
    {
      id: "method",
        ...
        <div className="flex justify-end items-center">
          {row.receipt_url ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-1 text-sm font-bold text-tertiary hover:text-tertiary/80 transition-colors p-2 rounded-md hover:bg-tertiary/10"
            >
