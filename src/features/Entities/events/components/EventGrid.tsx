    ...
    >
      ...
      >
        ...
        {event.status === "Upcoming" && featured && (
          <div className="absolute top-4 left-4 bg-on-surface text-surface px-3 py-1 kicker-label">
            Upcoming
          </div>
        )}
        {event.status === "Completed" && (
          <div className="absolute top-4 left-4 bg-surface text-on-surface border border-outline-variant/30 px-3 py-1 kicker-label">
            Past Event
          </div>
        )}
      </div>
        ...
        {featured ? (
          <div className="flex items-center justify-between mt-auto pt-6 border-t border-outline-variant/30">
            <div className="flex flex-wrap items-center gap-4">
              {event.joinUrl && (
                <Button
                  variant="default"
                  className="rounded-md bg-on-surface text-surface h-12 px-6 hover:bg-surface-variant uppercase text-xs tracking-wider font-bold transition-[transform,box-shadow,border-color] duration-300 ease-out"
                  asChild
                >
              ...
              {event.donationUrl && (
                <Button
                  variant="outline"
                  className="rounded-md border-on-surface text-on-surface h-12 px-6 hover:bg-on-surface hover:text-surface uppercase text-xs tracking-wider font-bold transition-[transform,box-shadow,border-color] duration-300 ease-out"
                  asChild
                >
            ...
            {event.capacity && (
              <div className="hidden sm:flex items-center gap-2 kicker-label text-on-surface-variant">
                <span>Capacity:</span>
          ...
          <div className="flex items-center justify-between mt-auto pt-6 border-t border-outline-variant/30">
            {event.joinUrl && (
              <Link
                href={event.joinUrl}
                className={`kicker-label flex items-center gap-2 group-hover:text-on-surface transition-colors ${
                  event.status === "Completed"
              ...
              >
                {event.actionLabel}{" "}
                <span className="material-symbols-outlined text-sm">{event.actionIcon}</span>
              </Link>
            )}
            {event.donationUrl && (
              <Button
...
export function EventGrid({ events }: { events: EventItem[] }) {
  return (
    <StaggerGroup
      y={28}
      className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12 mb-section-gap"
    >
