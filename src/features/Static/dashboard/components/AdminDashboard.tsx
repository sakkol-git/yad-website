    ...
    if (daysDifference === 0) {
      ...
      if (hoursDifference === 0) {
        const minDiff = Math.round(
          (new Date(dateString).getTime() - new Date().getTime()) / (1000 * 60),
        );
    ...
    <div className="flex-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-surface-container-lowest border border-outline-variant/30 rounded-md p-6   hover:-translate-y-1 transition-transform duration-300">
          ...
          <h3 className="text-3xl font-bold text-primary">
            {formatCurrency(metrics.monthlyDonations)}
          </h3>
        </div>
        ...
        <div className="bg-surface-container-lowest border border-outline-variant/30 rounded-md p-6   hover:-translate-y-1 transition-transform duration-300 relative overflow-hidden">
          ...
          <div className="flex justify-between items-start mb-4">
            <div
              className={`w-12 h-12 rounded-md ${metrics.pendingHomestays > 0 ? "bg-error-container/50 text-error" : "bg-surface-variant text-on-surface-variant"} flex items-center justify-center`}
            >
              <span
                className="material-symbols-outlined text-2xl"
                style={{ fontVariationSettings: "'FILL' 1" }}
            ...
            {metrics.pendingHomestays > 0 ? (
              <span className="bg-error/10 text-error px-2.5 py-1 rounded-sm text-xs font-bold flex items-center gap-1">
                <span className="material-symbols-outlined text-sm">priority_high</span> Action
                Needed
              </span>
            ) : (
              <span className="bg-surface-variant text-on-surface-variant px-2.5 py-1 rounded-sm text-xs font-bold flex items-center gap-1">
                <span className="material-symbols-outlined text-sm">check</span> All Clear
              </span>
            )}
          </div>
          <p className="text-sm font-medium text-on-surface-variant mb-1">Pending Homestays</p>
          <h3
            className={`text-3xl font-bold ${metrics.pendingHomestays > 0 ? "text-error" : "text-primary"}`}
          >
            {metrics.pendingHomestays}
          </h3>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 bg-surface-container-lowest border border-outline-variant/30 rounded-md p-7  ">
          ...
          <div className="space-y-1">
            ...
            {users.map((user: any) => (
              <div
                key={user.id}
                className="flex items-start gap-4 p-3 rounded-md hover:bg-surface-container/50 transition-colors group cursor-pointer"
              >
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-start">
                    ...
                    <span className="text-xs font-medium text-on-surface-variant bg-surface px-2 py-0.5 rounded-md">
                      {formatTimeAgo(user.created_at)}
                    </span>
                  </div>
                  <p className="text-sm text-on-surface-variant mt-1">
                    Assigned role:{" "}
                    <span className="font-medium text-secondary capitalize">{user.role}</span>
                  </p>
                </div>
              </div>
            ))}
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-on-surface-variant mt-1">
                    ...
                    {donation.first_name
                      ...
                      : "anonymous donor"}{" "}
                    via {donation.method?.toUpperCase()}
                  </p>
                </div>
              </div>
            ))}
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-surface-container-lowest border border-outline-variant/30 rounded-md p-7  ">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Link
                href="/admin/members"
                className="bg-surface border border-outline-variant/30 rounded-md p-4 flex flex-col items-center justify-center gap-2 hover:border-primary hover:bg-surface-container-low transition-[transform,box-shadow,border-color] duration-300 ease-out group"
              >
                <span
                  className="material-symbols-outlined text-[28px] text-primary group-hover:-translate-y-1 transition-transform"
                  style={{ fontVariationSettings: "'FILL' 1" }}
                >
                  group_add
                </span>
              ...
              <Link
                href="/admin/events"
                className="bg-surface border border-outline-variant/30 rounded-md p-4 flex flex-col items-center justify-center gap-2 hover:border-tertiary hover:bg-surface-container-low transition-[transform,box-shadow,border-color] duration-300 ease-out group"
              >
                <span
                  className="material-symbols-outlined text-[28px] text-tertiary group-hover:-translate-y-1 transition-transform"
                  style={{ fontVariationSettings: "'FILL' 1" }}
                >
                  event_note
                </span>
              ...
              <Link
                href="/admin/donors"
                className="bg-surface border border-outline-variant/30 rounded-md p-4 flex flex-col items-center justify-center gap-2 hover:border-secondary hover:bg-surface-container-low transition-[transform,box-shadow,border-color] duration-300 ease-out group"
              >
                <span
                  className="material-symbols-outlined text-[28px] text-secondary group-hover:-translate-y-1 transition-transform"
                  style={{ fontVariationSettings: "'FILL' 1" }}
                >
                  diversity_1
                </span>
              ...
              <Link
                ...
                className="bg-error/5 border border-error/20 rounded-md p-4 flex flex-col items-center justify-center gap-2 hover:bg-error/10 transition-[transform,box-shadow,border-color] duration-300 ease-out group relative"
              >
                <span className="absolute top-2 right-2 w-2 h-2 bg-error rounded-full animate-pulse"></span>
                <span
                  className="material-symbols-outlined text-[28px] text-error group-hover:-translate-y-1 transition-transform"
                  style={{ fontVariationSettings: "'FILL' 1" }}
                >
                  fact_check
                </span>
          ...
          <div className="bg-surface-container-lowest border border-outline-variant/30 rounded-md p-6   flex-1">
            <h3 className="text-base font-bold text-on-surface mb-4">System Status</h3>
