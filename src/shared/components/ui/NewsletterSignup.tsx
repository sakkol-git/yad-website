        ...
        <div className="flex flex-col lg:flex-row items-center gap-10">
          <div className="flex-1 w-full flex flex-col items-center lg:items-start">
            <div className="inline-flex items-center gap-4 mb-4">
              ...
              <span className="kicker-label text-primary">Newsletter</span>
            </div>
            ...
            <p className="font-light text-base text-on-surface-variant max-w-md leading-relaxed">
              Subscribe to our newsletter for updates on our impact, inspiring stories, and ways to
              get involved.
            </p>
          </div>
          ...
          <div className="w-full lg:w-auto flex-1 max-w-md">
            {status === "success" ? (
              ...
              <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3">
                ...
                >
                  <span
                    className={`absolute inset-0 flex items-center justify-center transition-opacity duration-150 ${status === "loading" ? "opacity-0" : "opacity-100"}`}
                  >
                    Subscribe
                  </span>
                  <span
                    className={`absolute inset-0 flex items-center justify-center gap-2 transition-opacity duration-150 ${status === "loading" ? "opacity-100" : "opacity-0"}`}
                    aria-hidden="true"
                  >
            ...
            <p
              ...
              role="alert"
              aria-live="polite"
              className={`text-xs mt-3 flex items-center justify-center lg:justify-start gap-1 transition-opacity duration-200 ${
                status === "error" ? "text-error opacity-100" : "opacity-0 pointer-events-none"
              }`}
            >
              {status === "error" && (
