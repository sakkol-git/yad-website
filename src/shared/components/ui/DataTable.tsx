        ...
        <div className="relative">
          ...
          {showColumnToggle && (
            <div className="absolute right-0 top-full mt-2 w-48 bg-surface-container-lowest border border-outline-variant/30 rounded-md shadow-lg z-10 p-2 animate-in fade-in zoom-in-95">
              <div className="text-xs font-bold text-on-surface-variant uppercase tracking-wider mb-2 px-2">
                Toggle Columns
              </div>
              {colsWithId
                ...
                .map((col) => (
                  <label
                    key={col.id}
                    className="flex items-center gap-2 px-2 py-1.5 hover:bg-surface rounded-md cursor-pointer text-sm text-on-surface"
                  >
                    <input
                      type="checkbox"
      ...
      <div className="rounded-md border border-surface-variant/50 overflow-hidden bg-surface shadow-sm flex flex-col relative">
            ...
            <TableHeader className="bg-surface-container-lowest sticky top-0 z-10 shadow-sm">
              <TableRow className="hover:bg-transparent">
                {activeColumns.map((col, i) => (
                  <TableHead
                    key={col.id}
                    className="font-bold text-on-surface-variant whitespace-nowrap"
                  >
                    {col.header}
                  </TableHead>
                ))}
        ...
        {count !== undefined && count !== null && count > 0 && (
          <div className="p-4 border-t border-surface-variant/30 flex flex-col sm:flex-row items-center justify-between bg-surface-bright text-on-surface-variant font-bold text-sm mt-auto gap-4">
            <div>
              Showing <span className="text-on-surface">{(page - 1) * 10 + 1}</span> to{" "}
              <span className="text-on-surface">{Math.min(page * 10, count)}</span> of{" "}
            ...
            <div className="flex gap-2 items-center">
              ...
              <div className="w-8 h-8 rounded-md bg-secondary-container text-on-secondary-container flex items-center justify-center shadow-sm">
                {page}
              </div>
              {page + 1 <= totalPages && (
                <Link
