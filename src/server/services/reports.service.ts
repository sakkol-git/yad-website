...
export class ReportsService {
  ...
  async uploadReport(
    ...
    // Get public URL
    const { data: publicUrlData } = adminClient.storage.from(STORAGE_BUCKET).getPublicUrl(filePath);
    ...
    await auditLogger.logAction(supabase, user.id, "annual_reports", "UPLOAD", {
      id: report.id,
      year: validated.year,
    });
  ...
  async updateReport(
    ...
    await auditLogger.logAction(supabase, user.id, "annual_reports", "UPDATE", {
      id,
      ...validated,
    });
