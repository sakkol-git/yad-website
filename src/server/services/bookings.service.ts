...
export class BookingsService {
  ...
  async getBookings(supabase: SupabaseClient<Database>, input: GetBookingsInput, isAdminRoute: boolean = false) {
    if (isAdminRoute) {
      await requireAdmin(supabase);
    }
    const validatedInput = getBookingsSchema.parse(input);
    return this.repository.getPaginated(supabase, validatedInput.page, validatedInput.limit, validatedInput.search, validatedInput.status, validatedInput.paymentStatus);
  }
