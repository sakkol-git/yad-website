...
export class MembersRepository extends BaseRepository<"members"> {
  ...
  async getPublicMembersByCategory(supabase: SupabaseClient<Database>, category: string) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  async getPublicMemberBySlug(supabase: SupabaseClient<Database>, slug: string) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const sb = supabase as any;
    const { data, error } = await sb
      .from("members")
