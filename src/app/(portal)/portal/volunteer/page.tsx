...
import { VolunteerPortal, PortalEvent, PortalEventVolunteer } from '@/features/Static/volunteer/components/VolunteerPortal';

export default async function PortalVolunteerPage() {
  const { upcomingEvents, myVolunteers } = await getUserVolunteerEvents();

  const signedUpEventIds = (myVolunteers as unknown as PortalEventVolunteer[])?.map((v) => v.event_id) || [];
  const availableEvents = (upcomingEvents as unknown as PortalEvent[])?.filter((e) => !signedUpEventIds.includes(e.id)) || [];
